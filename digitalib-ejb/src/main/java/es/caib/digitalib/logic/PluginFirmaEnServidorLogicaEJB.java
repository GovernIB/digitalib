...
@RunAs("DIB_ADMIN")
public class PluginFirmaEnServidorLogicaEJB extends AbstractPluginLogicaEJB<ISignatureServerPlugin>
        implements PluginFirmaEnServidorLogicaService {
    ...
    public Fitxer firmarFitxerAmbApiFirmaEnServidor(TransaccioJPA transaccio, Fitxer fitxer, Locale locale,
            String userApp, String userPerson) {
        ...
        if (confFirma.getTipusFirmaID() == Constants.TIPUSFIRMA_PADES) {
            ...
            if (!ScanWebDocument.SCANTYPE_MIME_PDF.equalsIgnoreCase(scanType)) {
                transaccio.setEstatCodi(ScanWebSimpleStatus.STATUS_FINAL_ERROR);
                transaccio.setEstatMissatge("El perfil de firma PADES només pot signar documents PADES "
        ...
        try {
            ...
            // Cridades de Plugin
            final String parametresText = "Tipus: " + signType + "\nMode:"
                    + (signMode == FileInfoSignature.SIGN_MODE_IMPLICIT ? "Implicit/Attached" : "Explicid/Detached");
            monitorIntegracions = pluginCridada.preCridada(confFirma.getPluginFirmaServidorID(), "filter,signDocuments",
            ...
            if (epes) {
                ...
                policyInfoSignature.setPolicyIdentifierHashAlgorithm(confFirma.getPolicyIdentifierHashAlgorithm()); // "SHA1");
            ...
            CommonInfoSignature commonInfoSignature = new CommonInfoSignature(languageUI, filtreCertificats, username,
                    administrationID);
            ...
            final int signaturesTableLocation = getPosicioTaulaDeFirmes(confFirma.getPosicioTaulaFirmesID());
            ...
            FileInfoSignature fileInfo = new FileInfoSignature(signID, source, previusSignatureDetachedFile,
                    FileInfoSignature.PDF_MIME_TYPE, name, reason, location, signerEmail, signNumber, languageDoc,
            ...
            if (error != null) {
                ...
                transaccio.setEstatMissatge(StringUtils.truncate(
                        "XYZ ZZZ Plugin " + plugin.getName(locale) + " No passa el filtre. Raó:  " + error, 2990));
            ...
            fitxerSignat = new FitxerBean("", FileInfoSignature.PDF_MIME_TYPE, nom, fileSigned.length());
            ...
            java.lang.String eniPerfilFirma = epes ? FirmaSimpleSignedFileInfo.SIGNPROFILE_EPES
                    : FirmaSimpleSignedFileInfo.SIGNPROFILE_BES;
            ...
            InfoSignaturaJPA infoSign = new InfoSignaturaJPA(signOperation, signType, signAlgorithm, signMode,
                    signaturesTableLocation, userRequiresTimeStamp, epes, eniTipoFirma, eniPerfilFirma, eniRolFirma,
