...
<script>
 ...
 function onChangeTipusFirma(combo) {
     ...
     if (value == <%=Constants.TIPUS_FIRMA_EN_SERVIDOR_SENSE%>) { 
       document.getElementById("<%=PerfilFields.CONFIGURACIOFIRMAID.fullName.replace('.', '_') %>_rowid").style.display = 'none';
       ...
       document.getElementById("<%=PerfilFields.CONFIGURACIOFIRMAID.fullName.replace('.', '_') %>_rowid").style.display = '';
       document.getElementById("<%=PerfilFields.APISIMPLEID.fullName.replace('.', '_') %>_rowid").style.display = 'none';
     }
