...
<un:useConstants var="TransaccioMultipleFields" className="es.caib.digitalib.model.fields.TransaccioMultipleFields"/>
        ...
        <c:if test="${ __entry.key < 0  && ((empty __entry.value.searchBy)? true : !gen:contains(__theFilterForm.hiddenFields, __entry.value.searchBy)) && ((empty __entry.value.groupBy )? true : !gen:contains(__theFilterForm.hiddenFields, __entry.value.groupBy ))}">
          <td>
             <c:if test="${not empty __entry.value.valueMap }">
               <c:out escapeXml="${__entry.value.escapeXml}" value="${__entry.value.valueMap[transaccioMultiple.transmultipleid]}" />
             </c:if>
        ...
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,TransaccioMultipleFields.FITXERESCANEJATID)}">
          <td>
            <c:if test="${not empty transaccioMultiple.fitxerEscanejat}">
              <a target="_blank" href="<c:url value="${dib:fileUrl(transaccioMultiple.fitxerEscanejat)}"/>">${transaccioMultiple.fitxerEscanejat.nom}</a>
            </c:if>
           </td>
        </c:if>
