...
public class AuthenticationSuccessListener implements
  ...
  public synchronized void onApplicationEvent(InteractiveAuthenticationSuccessEvent event) {
    ...
    try {
      UsuariPersonaLocal usuariPersonaEjb = null;
      usuariPersonaEjb = getUsuariPersonaEJB();
      Long usuariPersonaID = usuariPersonaEjb.executeQueryOne(UsuariPersonaFields.USUARIPERSONAID, UsuariPersonaFields.USERNAME.equal(username));
      
      
      if (usuariPersonaID == null) {
        usuariPersona = null;
      } else {
        usuariPersona = usuariPersonaEjb.findByPrimaryKey(usuariPersonaID);
      }
    } catch (Throwable e) {
      // TODO traduccio
      throw new LoginException("No puc accedir al gestor d´obtenció de" +
              " informació de usuari per " + username + ": " + e.getMessage(), e);
    }
     ...
     // if (containsRoleAdmin) { // || Configuracio.isCAIB() || PropietatGlobalUtil.getDefaultEntity() != null) {
        try {
          ...
          if (info != null) {
            ...
            usuariPersona = (UsuariPersonaJPA)usuariPersonaEjb.create(usuariPersona);
      ...
      if (containsRoleUser) {        
        ...
        for (RoleUsuariEntitat roleUsuariEntitat : rolesEntitat) {
          ...
          if (usuariAplicacioPerPerticionsIsNull && 
            ...
            log.warn("Error de seguretat: L'usuari " + username + " té el role virtual "
...
*/
    ...
    // create a new authentication token
    LoginInfo loginInfo;
    loginInfo = new LoginInfo(user, usuariPersona, new HashSet<GrantedAuthority>(roles), necesitaConfigurar);
  ...
  protected static UsuariPersonaLocal usuariPersonaEjb;
  
  public static UsuariPersonaLocal getUsuariPersonaEJB() throws I18NException {

    if (usuariPersonaEjb == null) {
      try {
        usuariPersonaEjb = (UsuariPersonaLocal) new InitialContext()
            .lookup(UsuariPersonaLocal.JNDI_NAME);
      } catch (Throwable e) {
       
        throw new I18NException(e, "error.unknown",
            new I18NArgumentString("No puc instanciar " + "UsuariPersonaLogicaLocal" + ": " + e.getMessage()));
      }
    }
    return usuariPersonaEjb;
  }
