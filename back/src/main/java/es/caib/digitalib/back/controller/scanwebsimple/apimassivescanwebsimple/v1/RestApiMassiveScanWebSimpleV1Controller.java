...
import org.fundaciobit.apisib.apimassivescanwebsimple.v1.beans.MassiveScanWebSimpleFormMetadatas;
...
public class RestApiMassiveScanWebSimpleV1Controller extends RestApiScanWebUtils
  ...
  public MassiveScanWebSimpleFile getSeparatorPage(String locale) throws Exception {
    ...
    MassiveScanWebSimpleFile msf = new MassiveScanWebSimpleFile(nom, "application/pdf", data);
  ...
  public String startTransaction(
      MassiveScanWebSimpleStartTransactionRequest startTransactionRequest) throws Exception {
  ...
  public MassiveScanWebSimpleSubtransactionResult getSubTransactionResult(
    ...
    try {
      ...
      if (status.getStatus() != MassiveScanWebSimpleStatus.STATUS_FINAL_OK) {
        return new MassiveScanWebSimpleSubtransactionResult(transactionID, transactionWebID,
            status);
      }

      MassiveScanWebSimpleFormMetadatas formMetadatas;
      {
        ...
        if (metadades != null && !metadades.isEmpty()) {
          for (Metadada metadada : metadades) {
            additionalMetadatas
                .add(new MassiveScanWebSimpleKeyValue(metadada.getNom(), metadada.getValor()));
          }
        }

        if (transaccio.getInfoScanPaperSize() != null) {
          additionalMetadatas.add(new MassiveScanWebSimpleKeyValue(
              MetadataConstants.PAPER_SIZE, transaccio.getInfoScanPaperSize()));
        }

        if (transaccio.getInfoScanDataCaptura() != null) {
          additionalMetadatas
              .add(new MassiveScanWebSimpleKeyValue(MetadataConstants.ENI_FECHA_INICIO,
                  ISO8601.dateToISO8601(transaccio.getInfoScanDataCaptura())));
        }

        // objecte Complet
        formMetadatas = new MassiveScanWebSimpleFormMetadatas(transactionName,
            functionaryUsername, signatureParameters, arxiuRequiredParameters,
            arxiuOptionalParameters, additionalMetadatas);
      ...
      final MassiveScanWebSimpleScannedFileInfo scannedFileInfo;
      {
        ...
        scannedFileInfo = new MassiveScanWebSimpleScannedFileInfo(pixelType, pppResolution,
            formatFile, ocr);
      ...
      if (perfilFirmaOArxiu) {
...

        {
          ...
          signedFileInfo = new MassiveScanWebSimpleSignedFileInfo(signOperation, signType,
      ...
      if (tipusPerfil == Constants.PERFIL_US_CUSTODIA_INFO) {
...

        {
          ...
          if (custody.getCustodyId() != null) {
            ...
            arxiuInfo = new MassiveScanWebSimpleArxiuInfo(expedientID, documentID, csv,
      ...
      MassiveScanWebSimpleSubtransactionResult fssr = new MassiveScanWebSimpleSubtransactionResult(
          transactionID, transactionWebID, status, formMetadatas, scannedFile, scannedFileInfo,
  ...
  public MassiveScanWebSimpleSubTransactionsOfTransaction getSubTransactionsOfTransaction(
    ...
    try {
      ...
      massivesubs = new MassiveScanWebSimpleSubTransactionsOfTransaction(transactionWebID,
          subtransacions);
      ...
      try {
        ...
        log.error(
            "Error intentant esbrinar el locale de la transaccio web " + transactionWebID);
      }
