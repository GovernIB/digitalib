...
es.caib.digitalib.plugins.signatureserver.miniappletinserver.defaultAliasCertificate=anadaljks', 1, 1, 'SIGN_MINIAPPLET_IN_SERVER', NULL, 'Plugin de Firma de Servidor emprant MiniApplet', 'Plugin de Firma de Servidor emprant MiniApplet');
...
es.caib.digitalib.plugins.scanweb.dynamicwebtwain.version=12.2', 1, 0, 'SCAN_DWT', 0, 'DynamicWebTwain', 'DynamicWebTwain');
...
es.caib.digitalib.plugins.scanweb.iecisa.closewindowwhenfinish=true', 1, 0, 'SCAN_JNLP', 1, 'Escaneig JNLP', 'Escaneig JNLP');
...
# Per desenvolupament
es.caib.digitalib.plugins.signatureserver.afirmalibs.debug=true', 1, 1, 'SIGN_AFIRMA_LIBS', 2, 'Plugin de Firma en Servidor emprant llibreries de @firma', 'Plugin de Firma en Servidor emprant llibreries de @firma');
INSERT INTO dib_plugin(pluginid, classe, properties, actiu, tipus, codi, ordre, nom, descripcio) VALUES (1234, 'org.fundaciobit.pluginsib.scanweb.file.FileScanWebPlugin', NULL, 1, 0, 'SCAN_FILE', 2, 'Seleccio Fitxer', 'Seleccio Fitxer');
...
es.caib.digitalib.plugins.signatureserver.afirmaserver.authorization.ks.cert.password=<<PASSWORD>>', 1, 1, 'SIGN_AFIRMA_FEDERAT', 1, 'Plugin de Firma en Servidor emprant @firma federat', 'Plugin de Firma en Servidor emprant @firma federat');
...
es.caib.digitalib.plugins.timestamp.afirmarfc.server.trustkeystore.password=123456789', 1, 4, 'TIME_AFIRMATS', 1, 'Afirma TS@', 'Afirma TS@');
...
# ${csv} => CSV | ${custodyID} => custodyID | ${custodyID_URLEncode} => URLEncode(custodyID) | ${custodyID_Hash} => Hash(custodyID)
# No funciona
...
INSERT INTO dib_plugin(pluginid, classe, properties, actiu, tipus, codi, ordre, nom, descripcio) VALUES (1274,
'org.fundaciobit.plugins.documentcustody.filesystem.FileSystemDocumentCustodyPlugin', 
to_clob('#Class: org.fundaciobit.plugins.documentcustody.filesystem.FileSystemDocumentCustodyPlugin
...
# Web on es pot validar el CSV
#es.caib.digitalib.plugins.documentcustody.filesystem.csv_validation_web_expressionlanguage=http://10.215.216.175:8080/custodiadigitalib/index.jsp
...
#es.caib.digitalib.plugins.documentcustody.filesystem.automatic_metadata.4.valueEL=${transaccio.infoSignatura.eniTipoFirma}'), 1, 2, 'DOCCUST_FILESYSTEM', 10, 'Custòdia en Sistema de Fitxers', 'Custòdia en Sistema de Fitxers');
...
es.caib.digitalib.plugins.timestamp.catcertrfc.hashalgorithm=SHA-512', 1, 4, 'TIME_CATCERT', 2, 'Segellador de Temps de CatCert', 'Segellador de Temps de CatCert');
...
INSERT INTO dib_plugin(pluginid, classe, properties, actiu, tipus, codi, ordre, nom, descripcio) VALUES (2204, 'org.fundaciobit.plugins.documentcustody.alfresco.base.AlfrescoBaseDocumentCustodyPlugin', 
to_clob('# CLASS: org.fundaciobit.plugins.documentcustody.alfresco.base.AlfrescoBaseDocumentCustodyPlugin
...
# Opcional. Servei per generar la URL de Custodia. Si no es defineix retornarà null. 
# {0} => custodyID // {1} => URLEncode(custodyID) // {2} => Hash(custodyID)
es.caib.digitalib.plugins.documentcustody.alfresco.baseurl=http://custody.fundaciobit.org/custody/{2}
...
# Opcional. Servei Millorat per generar la URL de Custodia emprant EL.
# Variables adicionals: validationUrl_custodyID, validationUrl_custodyID_URLEncode i validationUrl_custodyID_Hash
es.caib.digitalib.plugins.documentcustody.alfresco.baseurl_expressionlanguage=http://custody.fundaciobit.org/custody/${registre.url}

# Opcional.Retorna una URL que apunta a la versió imprimible del document
# Valors de substitució:  
# ${custodyID} => custodyID | ${custodyID_URLEncode} => URLEncode(custodyID) | ${custodyID_Hash} => Hash(custodyID)
#es.caib.exemple.plugins.documentcustody.alfresco.printable_file_url_expressionlanguage=http://custody.fundaciobit.org/custody/printable/${registre.url}

# Opcional.Retorna una URL que apunta a la versió ENI del document
# Valors de substitució:  
# ${custodyID} => custodyID | ${custodyID_URLEncode} => URLEncode(custodyID) | ${custodyID_Hash} => Hash(custodyID)
#es.caib.digitalib.plugins.documentcustody.alfresco.eni_file_url_expressionlanguage=http://custody.fundaciobit.org/custody/eni/${registre.url}
') || to_clob('

#  Retorna una plana web de validacio del document
# Valors de substitució:  
# ${custodyID} => custodyID | ${custodyID_URLEncode} => URLEncode(custodyID) | ${custodyID_Hash} => Hash(custodyID)
#es.caib.digitalib.plugins.documentcustody.alfresco.validation_file_url_expressionlanguage=http://custody.fundaciobit.org/custody/validationfile/${custodyID}
...
# es.caib.exemple.plugins.documentcustody.alfresco.csv_validation_web_expressionlanguage=http://custody.fundaciobit.org/custody/index.html


# Opcional. URL al document on s''explica com s''ha generat el CSV. Si no es defineix retornarà null. 
# es.caib.exemple.plugins.documentcustody.alfresco.csv_generation_definition=http://custody.fundaciobit.org/custody/csv_generation.pdf
...
es.caib.digitalib.plugins.documentcustody.alfresco.automatic_metadata.4.valueEL=${registre.tipoDocumental}'), 1, 2, 'DOCCUST_ALFRESCO', 3, 'Document Custody d''Alfresco 5.x', 'Document Custody d''Alfresco 5.x');


INSERT INTO dib_idioma(idiomaid, nom, suportat, ordre) VALUES ('ca', 'Català', 1, 0);
INSERT INTO dib_idioma(idiomaid, nom, suportat, ordre) VALUES ('es', 'Castellano', 1, 1);
...
INSERT INTO dib_usuariaplicacio(usuariaplicacioid, username, contrasenya, emailadmin, descripcio, actiu, rolescan, rolecoau, rolecust) VALUES (1007, 'testapp', NULL, 'digitalib@fundaciobit.org', NULL, 1, 1, 1, 1);
INSERT INTO dib_usuariaplicacio(usuariaplicacioid, username, contrasenya, emailadmin, descripcio, actiu, rolescan, rolecoau, rolecust) VALUES (1015, 'testappcaib', NULL, 'digitalib@fundaciobit.org', NULL, 1, 1, 1, 1);
