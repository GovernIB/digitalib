...
<body>
    ...
    <div class="row-fluid container main" style="max-width: none;">
        ...
        <ul class="nav nav-tabs custom-submenu">
            ...
            %>
            <li class="nav-item"><a class="nav-link ${(empty pipella)?'active' : '' }" href="<c:url value="/canviarPipella/"/>">
    ...
    --%>
            ...
            <sec:authorize access="hasRole('ROLE_USER')">
                ...
                <li class="nav-item"><a href="<c:url value="/canviarPipella/user"/>"
                        <%
        if (!Configuracio.isOcultarMenuInici()) {
        %>
                        ...
                        <fmt:message key="usuari.rol" />
                    </a></li>
            </sec:authorize>

            <sec:authorize access="hasRole('ROLE_ADMIN')">
                <li class="nav-item"><a href="<c:url value="/canviarPipella/admin"/>"
                ...
                <li class="nav-item"><a class="nav-link ${(pipella eq 'admindigitalib')?'active' : '' }"
                        href="<c:url value="/canviarPipella/admindigitalib"/>">
                        <fmt:message key="administrador.digitalib.rol" />
                    </a></li>

                <li class="nav-item"><a class="nav-link ${(pipella eq 'adminintegracions')?'active' : '' }"
                        href="<c:url value="/canviarPipella/adminintegracions"/>">
                        <fmt:message key="administrador.integracions.rol" />
                    </a></li>

                <li class="nav-item"><a class="nav-link ${(pipella eq 'webdb')?'active' : '' }"
                        href="<c:url value="/canviarPipella/webdb"/>"> WebDatabase</a></li>
            </sec:authorize>

            <c:if test="${dib:isDesenvolupament()}">
                <li class="nav-item"><a class="nav-link ${(pipella eq 'desenvolupament')?'active' : '' }"
