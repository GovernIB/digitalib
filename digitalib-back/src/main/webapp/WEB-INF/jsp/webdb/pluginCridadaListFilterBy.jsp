        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PluginCridadaFields.TIPUSRESULTAT)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
              ...
              <div class="input-group-prepend" style="min-width:200px">
                <form:select id="plugcrid_tipusResultat_select" path="tipusResultatSelect" cssClass="search-query input-medium form-control select2 select2-hidden-accessible" multiple="true" style="width:100%;" tabindex="-1" aria-hidden="true">
                    <c:forEach var="_entry" items="${__theFilterForm.mapOfValuesForTipusResultat}">
                      <option value="${_entry.key}" ${fn:contains(__theFilterForm.tipusResultatSelect, _entry.key)?'selected':''} >${_entry.value}</option>
                    </c:forEach>
                </form:select>
              </div>
              ...
              <script type="text/javascript">
                $(document).ready(function() {
                    $('#plugcrid_tipusResultat_select').select2({
                        closeOnSelect: false
                    });
