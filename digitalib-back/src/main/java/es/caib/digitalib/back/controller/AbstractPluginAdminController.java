...
public abstract class AbstractPluginAdminController extends PluginController {
    ...
    public void delete(HttpServletRequest request, Plugin plugin) throws I18NException {
        ...
        pluginLogicaEjb.deleteAndCheckPerfils(plugin,  getTipusDePlugin());
    ...
    public PluginForm getPluginForm(PluginJPA _jpa, boolean __isView, HttpServletRequest request, ModelAndView mav)
        ...
        if (pluginForm.isNou()) {
            ...
            if (p.isActiu()) {
                pluginForm.addAdditionalButton(new AdditionalButton(IconUtils.getWhite(IconUtils.ICON_BAN_CIRCLE),
                        "plugin.desactivar", getContextWebPlugin() + "/desactivarplugin/" + p.getPluginID(),
                        AdditionalButtonStyle.WARNING));
            } else {
                pluginForm.addAdditionalButton(new AdditionalButton(IconUtils.getWhite(IconUtils.ICON_OK_CIRCLE),
                        "plugin.activar", getContextWebPlugin() + "/activarplugin/" + p.getPluginID(),
    ...
    public String pluginActivate(HttpServletRequest request, HttpServletResponse response, @PathVariable
    ...
    public String pluginDeactivate(HttpServletRequest request, HttpServletResponse response, @PathVariable
    Long pluginid) throws I18NException {
        ...
        switch (tipusPlugin) {
            case Constants.TIPUS_PLUGIN_ARXIU:
                Where w1 = Where.AND(PerfilFields.PLUGINARXIUID.equal(p.getPluginID()),
            ...
            case Constants.TIPUS_PLUGIN_DOCUMENT_CUSTODY:
                Where w2 = Where.AND(PerfilFields.PLUGINDOCCUSTODYID.equal(p.getPluginID()),
                        PerfilFields.USPERFIL.greaterThan(0));
            ...
            case Constants.TIPUS_PLUGIN_SCANWEB:
                Where w5 = Where.AND(
                        Where.OR(PerfilFields.PLUGINSCANWEBID.equal(p.getPluginID()),
                                PerfilFields.PLUGINSCANWEB2ID.equal(p.getPluginID())),
                        PerfilFields.USPERFIL.greaterThan(0));
            ...
            default:
                throw new I18NException("genapp.comodi",
                        I18NUtils.tradueix("plugin.tipus.notrobat", String.valueOf(tipusPlugin)));
        }

        if (infoCodiNom.size() > 0) {
            String codiNom = infoCodiNom.get(0);
            String msg;
            if (tipusPlugin == Constants.TIPUS_PLUGIN_FIRMA_EN_SERVIDOR
                    || tipusPlugin == Constants.TIPUS_PLUGIN_SEGELLDETEMPS) {
    ...
    public List<StringKeyValue> getReferenceListForTipus(HttpServletRequest request, ModelAndView mav, Where where)
        ...
        __tmp.add(new StringKeyValue("" + Constants.TIPUS_PLUGIN_DOCUMENT_CUSTODY, "PLUGIN DOCUMENT CUSTODY"));
        __tmp.add(new StringKeyValue("" + Constants.TIPUS_PLUGIN_FIRMA_EN_SERVIDOR, "PLUGIN FIRMA EN SERVIDOR"));
