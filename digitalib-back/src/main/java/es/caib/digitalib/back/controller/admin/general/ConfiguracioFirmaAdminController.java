...
import es.caib.digitalib.persistence.TraduccioMapJPA;
import es.caib.digitalib.logic.ConfiguracioFirmaLogicaService;
...
public class ConfiguracioFirmaAdminController extends ConfiguracioFirmaController {
    ...
    protected ConfiguracioFirmaLogicaService configuracioFirmaLogicaEjb;
    ...
    @Override
    public ConfiguracioFirmaForm getConfiguracioFirmaForm(ConfiguracioFirmaJPA _jpa, boolean __isView,
    ...
    public ConfiguracioFirmaFilterForm getConfiguracioFirmaFilterForm(Integer pagina, ModelAndView mav,
        ...
        ConfiguracioFirmaFilterForm configuracioFirmaFilterForm = super.getConfiguracioFirmaFilterForm(pagina, mav,
                request);

        if (configuracioFirmaFilterForm.isNou()) {

            Set<Field<?>> hiddenFields = new HashSet<Field<?>>(Arrays.asList(ALL_CONFIGURACIOFIRMA_FIELDS));
    ...
    public List<StringKeyValue> getReferenceListForUsPoliticaDeFirma(HttpServletRequest request, ModelAndView mav,
        ...
        __tmp.add(new StringKeyValue("" + Constants.US_POLITICA_DE_FIRMA_NO_USAR,
                I18NUtils.tradueix("uspoliticadefirma." + Constants.US_POLITICA_DE_FIRMA_NO_USAR)));
        __tmp.add(new StringKeyValue("" + Constants.US_POLITICA_DE_FIRMA_OBLIGATORI_DEFINIT,
                I18NUtils.tradueix("uspoliticadefirma." + +Constants.US_POLITICA_DE_FIRMA_OBLIGATORI_DEFINIT)));
    ...
    public List<StringKeyValue> getReferenceListForTipusOperacioFirma(HttpServletRequest request, ModelAndView mav,
        ...
        __tmp.add(new StringKeyValue("" + Constants.TIPUS_OPERACIO_FIRMA_FIRMAR,
                I18NUtils.tradueix("operaciofirma." + Constants.TIPUS_OPERACIO_FIRMA_FIRMAR)));
        __tmp.add(new StringKeyValue("" + Constants.TIPUS_OPERACIO_FIRMA_COFIRMAR,
                I18NUtils.tradueix("operaciofirma." + +Constants.TIPUS_OPERACIO_FIRMA_COFIRMAR)));
        __tmp.add(new StringKeyValue("" + Constants.TIPUS_OPERACIO_FIRMA_CONTRAFIRMAR,
                I18NUtils.tradueix("operaciofirma." + +Constants.TIPUS_OPERACIO_FIRMA_CONTRAFIRMAR)));
        return __tmp;
    }
    ...
    public List<StringKeyValue> getReferenceListForPluginSegellatID(HttpServletRequest request, ModelAndView mav,
            ConfiguracioFirmaForm configuracioFirmaForm, Where where) throws I18NException {
    ...
    public List<StringKeyValue> getReferenceListForPluginSegellatID(HttpServletRequest request, ModelAndView mav,
        ...
        Where where2 = Where.AND(where, PluginFields.TIPUS.equal(Constants.TIPUS_PLUGIN_SEGELLDETEMPS));
    ...
    public List<StringKeyValue> getReferenceListForPluginFirmaServidorID(HttpServletRequest request, ModelAndView mav,
        ...
        Where where2 = Where.AND(where, PluginFields.TIPUS.equal(Constants.TIPUS_PLUGIN_FIRMA_EN_SERVIDOR));
    ...
    @Override
    public void postValidate(HttpServletRequest request, ConfiguracioFirmaForm configuracioFirmaForm,
        ...
        if (uac.getPluginFirmaServidorID() != 0) {
            if (uac.isIncloureSegellDeTemps()) {
                ...
                ISignatureServerPlugin issp = pluginFirmaServidorLogicaEjb
                        .getInstanceByPluginID(uac.getPluginFirmaServidorID());
    ...
    @Override
    public void delete(HttpServletRequest request, ConfiguracioFirma configuracioFirma) throws I18NException {
        configuracioFirmaLogicaEjb.deleteWithChecks(configuracioFirma);
    }
