...
import es.caib.digitalib.back.validator.webdb.AvisWebValidator;
...
public class AvisController
  ...
  @EJB(mappedName = es.caib.digitalib.ejb.IdiomaService.JNDI_NAME)
  protected es.caib.digitalib.ejb.IdiomaService idiomaEjb;

  @EJB(mappedName = es.caib.digitalib.ejb.AvisService.JNDI_NAME)
  ...
  public Map<Field<?>, GroupByItem> fillReferencesForList(AvisFilterForm filterForm,
    ...
    // Field configGrupID
    {
      _listSKV = getReferenceListForConfigGrupID(request, mav, filterForm, list, groupByItemsMap, null);
      _tmp = Utils.listToMap(_listSKV);
      filterForm.setMapOfConfiguracioGrupForConfigGrupID(_tmp);
      if (filterForm.getGroupByFields().contains(CONFIGGRUPID)) {
        fillValuesToGroupByItems(_tmp, groupByItemsMap, CONFIGGRUPID, false);
      };
    }


      fillValuesToGroupByItemsBoolean("genapp.checkbox", groupByItemsMap, BLOQUEJAR);
  ...
  public void exportList(@PathVariable("dataExporterID") String dataExporterID,
    ...
    __mapping.put(CONFIGGRUPID, filterForm.getMapOfConfiguracioGrupForConfigGrupID());
  ...
  public ModelAndView crearAvisGet(HttpServletRequest request,
    ...
    if (avisForm.getAvis().getDescripcio() == null){
      es.caib.digitalib.persistence.TraduccioJPA trad = new es.caib.digitalib.persistence.TraduccioJPA();
      for (es.caib.digitalib.model.entity.Idioma idioma : avisForm.getIdiomesTraduccio()) {
        trad.addTraduccio(idioma.getIdiomaID(), new es.caib.digitalib.persistence.TraduccioMapJPA());
      }
  ...
  public void fillReferencesForForm(AvisForm avisForm,
    ...
    // Comprovam si ja esta definida la llista
    if (avisForm.getListOfConfiguracioGrupForConfigGrupID() == null) {
      List<StringKeyValue> _listSKV = getReferenceListForConfigGrupID(request, mav, avisForm, null);

      if(_listSKV != null && !_listSKV.isEmpty()) { 
          java.util.Collections.sort(_listSKV, STRINGKEYVALUE_COMPARATOR);
      }
      avisForm.setListOfConfiguracioGrupForConfigGrupID(_listSKV);
    }
...
public java.lang.Long stringToPK(String value) {
 ...
 exceptionMsg };
  ...
  public List<StringKeyValue> getReferenceListForConfigGrupID(HttpServletRequest request,
       ModelAndView mav, AvisForm avisForm, Where where)  throws I18NException {
    if (avisForm.isHiddenField(CONFIGGRUPID)) {
      return EMPTY_STRINGKEYVALUE_LIST;
    }
    Where _where = null;
    if (avisForm.isReadOnlyField(CONFIGGRUPID)) {
      _where = ConfiguracioGrupFields.CONFIGURACIOGRUPID.equal(avisForm.getAvis().getConfigGrupID());
    }
    return getReferenceListForConfigGrupID(request, mav, Where.AND(where, _where));
  }
  ...
  public List<StringKeyValue> getReferenceListForConfigGrupID(HttpServletRequest request,
       ModelAndView mav, AvisFilterForm avisFilterForm,
       List<Avis> list, Map<Field<?>, GroupByItem> _groupByItemsMap, Where where)  throws I18NException {
    if (avisFilterForm.isHiddenField(CONFIGGRUPID)
       && !avisFilterForm.isGroupByField(CONFIGGRUPID)) {
      return EMPTY_STRINGKEYVALUE_LIST;
    }
    Where _w = null;
    if (!_groupByItemsMap.containsKey(CONFIGGRUPID)) {
      // OBTENIR TOTES LES CLAUS (PK) i despres només cercar referències d'aquestes PK
      java.util.Set<java.lang.Long> _pkList = new java.util.HashSet<java.lang.Long>();
      for (Avis _item : list) {
        if(_item.getConfigGrupID() == null) { continue; };
        _pkList.add(_item.getConfigGrupID());
        }
        _w = ConfiguracioGrupFields.CONFIGURACIOGRUPID.in(_pkList);
      }
    return getReferenceListForConfigGrupID(request, mav, Where.AND(where,_w));
  }
